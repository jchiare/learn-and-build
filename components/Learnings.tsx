...
export default function Learnings() {
  ...
  const { data: learnings = [] } = useQuery<Learning[]>({
    queryKey: ['learnings'],
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/learnings?id=${id}`, { method: 'DELETE' });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['learnings'] });
  ...
  const handleEdit = (learning: Learning) => {
    setFormData({ title: learning.title, content: learning.content });
